...
import InterviewFloatButton from '@/features/dashboard-core/components/InterviewFloatButton';
...
export default function CareerBlueprintPage() {
  ...
  useEffect(() => {
    try {
      ...
      // 支持任意岗位生成蓝图：从selectedJob/top1/top2/top3中读取
      const targetRole = mr.selectedJob?.role || mr.userSelectedJob?.role || mr.top1?.role || mr.role;
      const matchScore = mr.selectedJob?.score || mr.userSelectedJob?.score || mr.top1?.score || mr.score;
      if (targetRole) setMatchData(p => ({ ...p, targetRole, matchScore }));
  ...
  }, [matchData, session, toast]);

  // ── 自动生成报告（来自人岗匹配中心）
  useEffect(() => {
    const shouldAutoGenerate = sessionStorage.getItem('shouldAutoGenerateReport') === 'true';
    if (shouldAutoGenerate && !reportData && matchData.targetRole && matchData.matchScore) {
      // 清除标志，防止重复生成
      sessionStorage.removeItem('shouldAutoGenerateReport');
      // 稍微延迟，确保页面加载完成
  ...
  }, [matchData, reportData, handleGenerate]);
    ...
    <div className="max-w-[1600px] w-full mx-auto p-6 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-5">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[600px] max-h-[80vh]">
          <AIAssistantWidget variant="static" />
        </motion.div>

        {/* 返回人岗匹配中心提示 */}
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-200 p-4 shadow-sm">
          <div className="flex items-start gap-3">
            <div className="flex-shrink-0 mt-0.5">
              <Lightbulb className="w-5 h-5 text-blue-600" />
            </div>
            <div className="flex-1">
              <h4 className="font-semibold text-sm text-blue-900">💡 需要重新规划？</h4>
              <p className="text-sm text-blue-700 mt-1">
                如果想要根据其他岗位重新生成生涯蓝图，可以
                <a href="/person-post-matching" className="font-semibold text-blue-600 hover:text-blue-700 underline mx-1">返回人岗匹配中心</a>
                选择目标岗位后再次生成。
              </p>
              {/* 新增的明显提示 */}
              <p className="text-sm font-semibold text-orange-700 mt-2 bg-orange-50 px-3 py-2 rounded-lg border border-orange-200 shadow-sm">
                ⚠️ 重要提示：重新选择岗位后，请务必点击右上角【重新生成】按钮，才能生成最新的生涯蓝图
              </p>
            </div>
          </div>
        </motion.div>
      </div>
      <InterviewFloatButton />
    </div>
  );
}
