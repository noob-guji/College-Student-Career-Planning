...
const MOCK_HISTORY: (TestResultData & { date: string })[] = [
    {
        type: 'ESTJ',
        title: '总经理',
    ...
    },
    {
        ...
        description: '诗意、善良且利他，总是致力于正当的事业。',
        scores: { e: 20, i: 80, s: 30, n: 70, t: 35, f: 65, j: 15, p: 85 },
        strengths: ['同理心极强，善于理解他人感受', '慷慨且宽容，胸襟开阔', '极具创造力，想象力丰富', '充满激情，为自己的信念而战'],
        weaknesses: ['过于理想主义，容易对现实失望', '容易感到孤立，觉得自己被误解', '缺乏专注力，难以坚持完成枯燥任务', '极度害怕冲突，倾向于逃避对抗']
    }
];
...
export default function MBTILanding() {
    ...
    const handleOpenHistory = (data: TestResultData) => {
        setHistoryData(data);
        setIsTestModalOpen(true);
    ...
    return (
        <div className="flex flex-col h-full items-center px-4 py-10 relative overflow-hidden sm:overflow-visible">
            {/* Background elements */}
            <div className="absolute top-0 right-0 w-[40vw] h-[40vw] max-w-[500px] max-h-[500px] bg-slate-50 rounded-full blur-3xl opacity-50 z-0 -translate-y-1/2 translate-x-1/3"></div>
                                ...
                                >
                                    <div className="p-5 flex flex-col gap-3">
                                        <div className="px-1 pb-2 border-b border-slate-100 flex items-center justify-between">
                                            ...
                                            <span className="text-xs font-semibold px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full">2条记录</span>
                                        </div>
                                        ...
                                        {MOCK_HISTORY.map((item, i) => (
                                            ...
                                            >
                                                <div>
                                                    <h4 className="font-bold text-slate-800 group-hover:text-slate-900 transition-colors">{item.type} - {item.title}</h4>
                                                    <span className="text-xs text-slate-400 font-medium tracking-wide">{item.date}</span>
                                                </div>
                                                <ChevronRight className="w-5 h-5 text-slate-300 group-hover:text-slate-500 transition-colors group-hover:translate-x-1" />
                                            </div>
                                        ))}
                                    </div>
                                </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-4xl pb-10 relative z-10 px-2 lg:px-0">
                {PERSONALITY_TYPES.map((type) => (
                    ...
                    >
                        {/* Background gradient */}
                        <div className={`absolute inset-0 bg-gradient-to-b opacity-60 z-0 ${type.color}`}></div>
                        ...
                        <div className="relative z-10 flex flex-col items-center w-full">
                            ...
                            {/* Text Content */}
                            <h3 className="text-2xl font-black text-slate-900 mb-2 tracking-wide">
                                {type.title}
                            </h3>
                            <div className="text-xs font-bold text-slate-400 tracking-[0.2em] mb-4 uppercase">
                                {type.examples}
                            </div>
            ...
            {/* Existing Assessment Questionnaire Modal */}
            <MBTIModal 
            ...
            {/* New Individual Personality Type Details Modal */}
