...
export default function JobPortraitPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_PYTHON_API}/api/portraits?limit=10750`) // 目前先加载10000条，后续可以考虑分页加载或者增加筛选条件，避免一次性加载过多数据导致性能问题
      .then(r => r.json())
          ...
          <div className="flex-1 overflow-y-auto py-2 scrollbar-thin">
            ...
            {filtered.map((cat, ci) => (
              <div key={cat.name}>
                ...
                {/* 二级岗位列表 */}
                {openCats.has(ci) && cat.jobs.map((job, index) => (
                  <button
                    key={`job-${index}`}
                    onClick={() => setSelected(job)}
                    className={`w-full flex items-start gap-2 px-4 py-2.5 pl-9 text-left transition-colors border-l-4 ${selected === job
                        ? 'bg-amber-50 border-amber-300'
                        : 'bg-white border-transparent hover:bg-slate-50'}
                      `}
                  >
                    <div className="flex-1 min-w-0">
                      <div className={`text-sm truncate font-medium ${selected === job ? 'text-amber-700' : 'text-slate-800'}`}>
                        {job.job_title}
                      </div>
