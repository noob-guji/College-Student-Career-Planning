...
export default function DigitalHumanInterview() {
  ...
  const speakText = useCallback((text: string, reactionLevel?: ReactionLevel) => {
    ...
    ttsRef.current.speak(text, emotion);
  }, [playReaction, isMuted]);
  ...
  useEffect(() => {
    if (currentFeedback && phase !== 'idle') {
      ...
      speakText(currentFeedback.feedback, reactionLevel);
    }
  }, [currentFeedback, phase, speakText]);
