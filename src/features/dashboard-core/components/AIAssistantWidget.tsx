...
import { Bot, Send, User, ChevronDown, MessageSquareMore, Trash2 } from 'lucide-react';
...
function MessageBubble({ content, isTyping }: { content: string; isTyping?: boolean }) {
    if (isTyping) {
        return (
            <div className="flex gap-1.5 items-center h-5 px-1 py-0.5">
                <span className="w-1.5 h-1.5 bg-slate-400 rounded-full animate-[bounce_1s_infinite_0ms]" />
                <span className="w-1.5 h-1.5 bg-slate-400 rounded-full animate-[bounce_1s_infinite_150ms]" />
                <span className="w-1.5 h-1.5 bg-slate-400 rounded-full animate-[bounce_1s_infinite_300ms]" />
            </div>
        );
    }
    // Render newlines and markdown bold
...
export default function AIAssistantWidget({ variant = 'floating' }: { variant?: 'floating' | 'static' }) {
    ...
    useEffect(() => {
        if (isOpen || variant === 'static') scrollToBottom();
    ...
    useEffect(() => {
        if (variant === 'static') setIsOpen(true);
    }, [variant]);
    ...
    const chatWindow = (
        <div className={variant === 'floating'
            ...
            : "w-full h-[600px] max-h-[80vh] bg-white rounded-2xl border border-slate-200 flex flex-col overflow-hidden shadow-sm"}>
            {/* Header */}
            <div className="bg-[#111827] p-4 flex justify-between items-center text-white shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-indigo-100 flex items-center gap-1.5 mt-0.5">
                            <span className="w-1.5 h-1.5 bg-green-400 rounded-full shadow-[0_0_4px_rgba(74,222,128,0.8)] animate-pulse" />
                            在线咨询
                        </p>
                    </div>
                </div>
                <div className="flex items-center gap-1">
                    ...
                    {variant === 'floating' && (
                        <button onClick={() => setIsOpen(false)} className="p-1.5 hover:bg-white/15 rounded-lg transition-colors">
                    ...
                    <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            <div className={`p-3.5 rounded-2xl text-[14px] leading-relaxed relative ${msg.role === 'user'
                                ...
                                <MessageBubble content={msg.content} isTyping={msg.isTyping} />
                            </div>
                        </div>
                    </div>
                ))}
